...
main = do
  ...
  let reactedPolymer = length $ reactPolymer polymer
      bestReaction = minimum $ map (length . reactPolymer . (\c -> filter ((/=(toLower c)) . toLower) polymer)) ['a'..'z']
  putStrLn $ "Part 1: " ++ (show reactedPolymer)
